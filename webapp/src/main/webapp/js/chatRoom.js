var ChatRoom = function (chatRoomDivId, eventbus, events, userService, chatService) {
    ...
    var ChatComponent = function () {
        ...
        var _initialize = function (userLoggedInEvent) {
            ...
            $('<label/>').text("Welcome " + userLoggedInEvent.username).appendTo($('fieldset'));
            $('<br>').appendTo($('fieldset'));
            $('<br>').appendTo($('fieldset'));
            ...
            $('#' + createChatButtonId).click(function () {
                ...
                var chatData = {
                    ...
                    'owner': userLoggedInEvent.userId,
                    'token': userLoggedInEvent.tokenId
                };

                console.log('User ' + userLoggedInEvent.username + ' is trying to create chat ' + chatData.name);
            ...
            $('#' + joinChatButtonId).click(function () {
                ...
                var chatData = {
                    ...
                    'userId': userLoggedInEvent.userId,
                    'token': userLoggedInEvent.tokenId
                };

                console.log('User ' + userLoggedInEvent.username + ' is trying to join chat ' + chatData.chatName);
        ...
        var _updateAvailableChats = function (chatList) {
            ...
            for (var i = 0; i < allChats.length; i++) {
                $('#' + selectChatId).append($('<option/>').text(allChats[i]));
            }
        ...
        var _showErrorMessage = function (errorEvent) {
            ...
            $('#' + chatComponentId + '_createChatError').attr('style', 'color:red; font-size:14; font-family:"Calibri"')
                .text(errorEvent.errorMessage);
        };

        var _showChat = function (chatData) {

            var chatBoxId = chatRoomDivId + '_' + chatData.chatId;
            ...
            $('#' + chatBoxId).append($('<label/>')).text(chatData.chatName)
            ...
            var messages = $.map(JSON.parse(chatData.messageList), function (element) {
                return element;
            });

            if (messages.length !== 0) {
                for (var i = 0; i < messages.length; i++) {
                    $('#' + messagesId).append($('<li/>').text(messages[i]));
                }
            }
            ...
            $('#' + sendButtonId).click(function () {
                ...
                var updatedChatData = {
                    'token': chatData.tokenId,
                    'chatName': chatData.chatName,
                    'author': chatData.userId,
            ...
            $('#' + leaveButtonId).click(function () {
                ...
                console.log('User ' + chatData.username + ' is trying to left chat ' + chatData.chatName);
        ...
        var _updateMessages = function (chatData) {
            ...
            var messagesId = chatRoomDivId + '_' + chatData.chatId + '_messages';
            var messageInputId = chatRoomDivId + '_' + chatData.chatId + '_newMessage';
    ...
    eventbus.subscribe(events.ATTEMPT_TO_CREATE_CHAT, chatService.createChat);
